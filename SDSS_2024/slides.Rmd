...
- Questions on case evidence overall, algorithm evidence, examiner evidence, and firearms evidence as a field:

  - How reliable/scientific was the evidence? (7-point Likert Scale)
    - Extremely, Moderately, Weakly, Neither/nor...
...
- Participants Struggle with Numerical Values
...
- Categorical log scale for chance of guilt (Thompson, Kaasa, & Peterson, 2013)
  - 1 chance in (10/100/1,000) of guilt
...
- New Questions
    ...
    - If provided with $50, how much would you be willing to bet that the defendant committed the crime/did not commit the crime? (Numeric)
...
<center><figure>
<img src="images/Study2_Screenshot2.jpg" width="65%" />
  <figcaption>Characters developed by Richy Meleus</figcaption>
</figure></center>
...
- Chance of Innocence: `r round(correct_summary_df$Count[4]/(correct_summary_df$Count[4]+correct_summary_df$Count[3])*100, 2)`% consistent (`r correct_summary_df$Count[4]` out of `r correct_summary_df$Count[4]+correct_summary_df$Count[3]`)

- Chance of Guilt: `r round(correct_summary_df$Count[2]/(correct_summary_df$Count[2]+correct_summary_df$Count[1])*100, 2)`% consistent (`r correct_summary_df$Count[2]` out of `r correct_summary_df$Count[2]+correct_summary_df$Count[1]`)
...
- Consistent with Thompson, Kaasa, & Peterson, 2013
...
- `r round(sum(clean_results_merged$prob_mult_consistency=="consistent")/length(clean_results_merged$prob_mult_consistency)*100, 2)`% of individuals selected consistent values
...
- Does Chance of Innocence = 1 - Chance of Guilt?
